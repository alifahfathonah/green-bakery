              ...
              <table id="transaksi" class="table table-bordered table-striped">
                ...
                <?php while($row = $data_pengiriman->fetch_assoc()) : ?>
                  <tr>
                    ...
                    <td>
                     <?php if ($row['status'] == 1) : ?>
                        <span class="label label-primary">Siap Dikirim</span>
                    <?php elseif ($row['status'] == 2) : ?>
                        <span class="label label-success">Telah Dikirim</span>
                    <?php else: ?>
                        <span class="label label-default">Belum Di Proses</span>
                    <?php endif ?>
                    </td>
                    <td>
                    <?php if ($row['tanggal_dikirim']) : ?>
                        <?=$row['tanggal_dikirim']?>
                    <?php else: ?>
                        Belum Di Kirim
                    <?php endif ?>
                    ...
                    <td>
                    <?php if ($row['status'] == 1) : ?>
                        <button type="button" id="<?=$row['id_transaksi']?>" class="btn btn-sm btn-flat btn-primary kirim">Kirim Pesanan</button>
                    <?php elseif ($row['status'] == 2) : ?>
                        <button type="button" class="btn btn-sm btn-flat btn-success" disabled>Telah dikirim</button>
                        <button type="button" id="<?=$row['id_transaksi']?>" class="btn btn-sm btn-flat btn-danger batal">Batalkan</button>
                    <?php else: ?>
                        <button type="button" class="btn btn-sm btn-flat btn-default" disabled>Kirim Pesanan</button>
                    <?php endif ?>
                    </td>
                  </tr>
                  <?php endwhile ?>
...
<script>
  ...
  $(".kirim").click(function(){
    ...
    if(x){
      $.post('<?=$host;?>/panel/rubah_status_pengiriman',{id_transaksi:id, status: status}).done(function(){
  ...
  $(".batal").click(function(){
    ...
    if(x){
      $.post('<?=$host;?>/panel/rubah_status_pengiriman',{id_transaksi:id, status: status}).done(function(){
        alert('Pesanan telah di batalkan.');
        location.reload();
      })
    }
  })
